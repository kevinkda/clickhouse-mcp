...
log = logging.getLogger("clickhouse_mcp._platform")
...
def _notify_windows(title: str, message: str) -> None:  # pragma: no cover - windows-only branch
    ...
    try:
        ...
        notification.notify(title=title, message=message, app_name="ClickHouse MCP", timeout=5)
        return
    ...
    script = (
        ...
        '[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("ClickHouse MCP")'
