"""Cross-platform shim tests (``clickhouse_mcp._platform``).
...
from clickhouse_mcp import _platform
...
def test_secure_chmod_windows_logs_warning_once(
    ...
    f = tmp_path / "win.bin"
    f.write_text("x")
    with caplog.at_level(logging.WARNING, logger="clickhouse_mcp._platform"):
        _platform.secure_chmod(f, 0o600)
...
def test_notify_desktop_macos_uses_osascript_when_available(
    ...
    _platform.notify_desktop("ClickHouse MCP", "self-test")
